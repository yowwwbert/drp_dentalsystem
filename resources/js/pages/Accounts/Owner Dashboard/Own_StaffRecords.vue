...
import staffRecordsData from '@/tempData/staffRecordsData.json';
...
const filteredStaff = computed(() => {
    let filtered = staffRecordsData.staff;
    if (searchQuery.value) {
        filtered = filtered.filter(staff =>
            staff.firstName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            staff.lastName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            staff.email.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            staff.phoneNumber.includes(searchQuery.value)
        );
    }
    if (selectedBranch.value !== 'Select Branch') {
        filtered = filtered.filter(staff => staff.branchName === selectedBranch.value);
    }
    return filtered;
});
...
const totalPages = computed(() => {
    return Math.max(1, Math.ceil(filteredStaff.value.length / itemsPerPage.value));
});
...
const handleViewStaff = (staffId: number) => {
    const staff = staffRecordsData.staff.find(s => s.id === staffId);
    selectedStaff.value = staff || null;
    showViewModal.value = true;
};

const handleEdit = () => {
    // For now, edit the first staff as an example
    const firstStaff = staffRecordsData.staff[0];
    if (firstStaff) {
        editForm.value = { 
            ...
            branchName: firstStaff.branchName
        };
    }
...
const addStaff = () => {
    ...
    console.log('Adding staff:', editForm.value);
    closeEditModal();
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 rounded-xl p-4 bg-gray-50 min-h-screen relative">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-4" :class="{ 'blur-[1px]': showViewModal || showEditModal }">
                <h1 class="text-3xl font-bold text-gray-900">Staff Records</h1>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-4" :class="{ 'blur-[1px]': showViewModal || showEditModal }">
                <div class="flex items-center gap-2">
                    ...
                    >
                        ...
                        <option v-for="branch in staffRecordsData.branches" :key="branch.id" :value="branch.name">{{ branch.name }}</option>
                    </select>
                </div>
                ...
                <table class="w-full min-w-[900px] border-separate border-spacing-0">
                    ...
                    <tbody>
                        <tr v-if="paginatedStaff.length === 0">
                            <td colspan="8" class="text-center py-8 text-gray-500 text-lg">No staff records found.</td>
                        </tr>
                        <tr v-for="staff in paginatedStaff" :key="staff.id" class="border-b last:border-b-0 hover:bg-gray-50">
                            <td class="py-3 px-4">{{ staff.id }}</td>
                            <td class="py-3 px-4">{{ staff.lastName }}</td>
                            <td class="py-3 px-4">{{ staff.firstName }}</td>
                            <td class="py-3 px-4">{{ staff.email }}</td>
                            <td class="py-3 px-4">{{ staff.phoneNumber }}</td>
                            <td class="py-3 px-4">{{ staff.position }}</td>
                            <td class="py-3 px-4">{{ staff.branchName }}</td>
                            <td class="py-3 px-4">
                                <button
            ...
            <div v-if="showViewModal" class="absolute inset-0 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
                    <h2 class="text-xl font-bold text-gray-900 mb-4">View Staff Record</h2>
                    <div v-if="selectedStaff" class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">ID</label>
                            <input type="text" :value="selectedStaff.id" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">First Name</label>
                            <input type="text" :value="selectedStaff.firstName" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                            <input type="text" :value="selectedStaff.lastName" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                            <input type="email" :value="selectedStaff.email" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                            <input type="text" :value="selectedStaff.phoneNumber" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Position</label>
                            <input type="text" :value="selectedStaff.position" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Branch</label>
                            <input type="text" :value="selectedStaff.branchName" readonly class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50" />
                        </div>
                    </div>
            ...
            <div v-if="showEditModal" class="absolute inset-0 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
                    <h2 class="text-xl font-bold text-gray-900 mb-4">{{ editForm.firstName ? 'Edit Staff Record' : 'Add New Staff' }}</h2>
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">First Name</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Position</label>
                            <select v-model="editForm.position" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-darkGreen-900">
